import{_ as e}from"./plugin-vue_export-helper-c27b6911.js";import{o as i,c as s,e as n}from"./app-58fd5dfd.js";const d={},l=n(`<h1 id="优化1、分页优化" tabindex="-1"><a class="header-anchor" href="#优化1、分页优化" aria-hidden="true">#</a> 优化1、分页优化</h1><h3 id="分页的实现" tabindex="-1"><a class="header-anchor" href="#分页的实现" aria-hidden="true">#</a> 分页的实现</h3><p>一般的分页查询使用简单的 limit 子句就可以实现。limit 格式如下：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>SELECT * FROM 表名 LIMIT [offset,] rows
...
select * from user limit 10000,100;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>结果：在查询记录时，如果查询记录量相同，偏移量超过 100 后就开始随着偏移量增大，查询时间急剧的增加。（这种分页查询机制，每次都会从数据库第一条记录开始扫描，越往后查询越慢，而且查询的数据越多，也会拖慢总查询速度。）</p><h3 id="分页优化策略" tabindex="-1"><a class="header-anchor" href="#分页优化策略" aria-hidden="true">#</a> 分页优化策略</h3><p><strong>优化 1: 通过索引进行分页</strong>，使用主键 id</p><p>直接进行 limit 操作 会产生全表扫描,速度很慢. Limit 限制的是从结果集的 M 位置处取出 N 条输出,其余抛弃.</p><p>假设 ID 是连续递增的,我们根据查询的页数和查询的记录数可以算出查询的 id 的范围，然后配合 limit 使用</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>EXPLAIN SELECT * FROM user WHERE id  &gt;= 100001 LIMIT 100;
