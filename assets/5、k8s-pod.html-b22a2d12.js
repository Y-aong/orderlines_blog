import{_ as e}from"./plugin-vue_export-helper-c27b6911.js";import{r as t,o as l,c as p,a as n,b as s,d as o,e as c}from"./app-58fd5dfd.js";const i={},u=n("h1",{id:"_5、k8s-pod",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#_5、k8s-pod","aria-hidden":"true"},"#"),s(" 5、k8s-pod")],-1),d=n("h3",{id:"一、pod概念",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#一、pod概念","aria-hidden":"true"},"#"),s(" 一、pod概念")],-1),r=n("em",null,"Pod",-1),k={href:"https://kubernetes.io/zh/docs/concepts/overview/what-is-kubernetes/#why-containers",target:"_blank",rel:"noopener noreferrer"},m=c(`<h3 id="二、pod特点" tabindex="-1"><a class="header-anchor" href="#二、pod特点" aria-hidden="true">#</a> 二、pod特点</h3><ul><li>pod对于容器有自律能力（pod自动重启失败的容器）</li><li>pod不能恢复自己</li><li>pod分为单容器和多容器</li><li>pod为其成员容器提供两种共享资源：网络和存储</li><li>一个pod由一个pause容器设置好整个pod里面所有的容器的网络和名称空间等信息</li></ul><h3 id="三、容器的生命周期" tabindex="-1"><a class="header-anchor" href="#三、容器的生命周期" aria-hidden="true">#</a> 三、容器的生命周期</h3><ul><li>pod启动，会依次启动所有的初始化容器，有一个失败，则pod都不能启动</li><li>接下来<strong>启动所有的应用容器</strong>（每一个应用容器都必须能一直运行起来），Pod开始正式工作，一个启动失败就会<strong>尝试重启Pod内的这个容器</strong>，Pod只要是NotReady，Pod就不对外提供服务了</li></ul><h3 id="四、pod的重启策略" tabindex="-1"><a class="header-anchor" href="#四、pod的重启策略" aria-hidden="true">#</a> 四、pod的重启策略</h3><p>Pod 的 <code>spec</code> 中包含一个 <code>restartPolicy</code> 字段，其可能取值包括 Always、OnFailure 和 Never。默认值是 Always。</p><p><code>restartPolicy</code> 适用于 Pod 中的所有容器。</p><h3 id="五、静态pod" tabindex="-1"><a class="header-anchor" href="#五、静态pod" aria-hidden="true">#</a> 五、静态pod</h3><p>在 <strong>/etc/kubernetes/manifests</strong> 位置放的所有Pod.yaml文件，机器启动kubelet自己就把他启动起来。</p><p>静态Pod一直守护在他的这个机器上</p><h3 id="六、prode探针机制-健康检查机制" tabindex="-1"><a class="header-anchor" href="#六、prode探针机制-健康检查机制" aria-hidden="true">#</a> 六、prode探针机制（健康检查机制）</h3><h4 id="三种探针" tabindex="-1"><a class="header-anchor" href="#三种探针" aria-hidden="true">#</a> 三种探针</h4><ul><li><p>启动探针：一次性成功探针，只要启动了就不用了</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code># 功能
